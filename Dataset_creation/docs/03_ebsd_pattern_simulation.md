...
| `EMEBSD_ni.nml` | 60×60, PC = `(4.337, 17.273, 1500.8)` matching the real camera | 4,125 real-scan-derived orientations (`ni_angles.txt`) | `Ni_EBSD_sim.h5` — **the one everything downstream uses** |

The pattern-center numbers in `EMEBSD_ni.nml` and the 4,125-line
`ni_angles.txt` both come from [`06_euler_angle_validation`](06_euler_angle_validation.md) —
this stage can't run meaningfully until that one supplies them first. That
dependency is what makes 06 the one loop in an otherwise linear pipeline: it
runs *before* this stage (to supply inputs) and *after* it (to check the
output) — see that page for the full explanation.

`EMEBSD.template` is a blank, generic reference copy — not read by anything.

## Visual check

`view_patterns.py` opens `Ni_EBSD.h5` (the smoke-test run) and plots a few
of its 5 patterns to `Ni_patterns.png`.
